    ...
    def get_links(nodes, current_path):
        ...
        for node in nodes:
            ...
            if '/c/' in url and url.strip():
                if category_url not in duplicate_checker:
                    links[full_path] = {"category_url": category_url, "api_url": api_url+url.split('/c/')[-1]}
                    duplicate_checker.add(category_url)
...
def main(url):
    ...
    if not has_categories():
        ...
        print(f"Category links extracted and saved Inserted {count} links.")
    else:
        print("Category links already exist. Skipping extraction.")
    flag=True
    while flag:
        category_links=get_pending_category_links()
        print("Pending categories: Links", len(category_links))
        print("*"*50)
        if not category_links:
            print("No category links found in the database.")
            break
        for data in category_links:
            api_url = data['product_api_url']
            
            product_links, total_products_count = parser_product_links(api_url)
            with open(f"products_{data['sub_id']}.json", "w", encoding="utf-8") as f:
                json.dump(product_links, f, indent=4, ensure_ascii=False)
            print(f"Total products for {data['sub_id']}: {total_products_count}",   f"\nproducts_links_saved{len(product_links)}")
            flag=False
            break
