...
def create_tables():
    ...
    product_link_table_sql = """
        CREATE TABLE IF NOT EXISTS product_links (
            ...
            CONSTRAINT fk_product_category
                FOREIGN KEY (category_source_id) REFERENCES category_Links (sub_id)
                ON DELETE CASCADE
        )
    """
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                "ALTER TABLE category_Links MODIFY COLUMN status ENUM('pending', 'processing', 'done', 'failed') NOT NULL DEFAULT 'pending'"
            )
            cursor.execute(
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            for key, category in json_data.items():
                print(category)
                cursor.execute(
                    "INSERT INTO category_Links (category_name, category_url, product_api_url) VALUES (%s, %s, %s)",
                    (
                        key,
...
def has_categories():
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            cursor.execute("SELECT COUNT(*) FROM category_Links")
...
def update_category_status(sub_id, status):
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            cursor.execute("UPDATE category_Links SET status = %s WHERE sub_id = %s", (status, sub_id))
        conn.commit()
